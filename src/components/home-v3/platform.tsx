...
import { Mockup } from "@/components/home-v3/mockups";
import { HOME_V3_PLATFORM } from "@/content/home-v3";
import type { HomeSectionMeta } from "@/lib/cms/home-content";

/**
 * 04 — IndiGeneUs.AI, with its relationship to Genetico said in plain words.
 *
 * The extraction demo lives here rather than in the hero, and runs itself once
 * as the section comes into view.
 *
 * What is new this round is below it: the four layers each frame a hand-built
 * mock-up of the screen they describe, in place of v2's text-only table. That
 * moved the grid from four columns to two (one in the middle band), because
 * at a quarter of the page width the panels were not legible.
 */
export function Platform({ section, num }: { section: HomeSectionMeta; num: string }) {
  const content = HOME_V3_PLATFORM;
    ...
    >
      <div className="max-w-site mx-auto">
        ...
        <div className="mt-[clamp(48px,6vw,72px)]">
          ...
          <div
            className="swipe-row nav:grid nav:grid-cols-1 mid:grid-cols-2 mt-[26px] gap-[18px]"
            style={{ "--swipe-col": "86%" } as React.CSSProperties}
          >
            {content.layers.map((layer, i) => (
              <div
                key={layer.title}
                className="border-rule rounded-card shadow-card flex min-w-0 flex-col gap-[13px] border bg-white px-[18px] pt-4 pb-5"
              >
                <Mockup kind={layer.mockup} />
