...
import { HOME_V3_HERO } from "@/content/home-v3";
...
/**
 * The third pass at the hero. Structurally it is v2's — one centred editorial
 * statement over the interactive dot grid, sticky so the fold lifts over it —
 * but the copy does the opposite job: it answers "what is Genetico?" in its
 * first sentence and names IndiGeneUs.AI rather than withholding it until
 * section 04.
 *
 * The sizing is v2's hard-won one: the block gives up spacing as the viewport
 * shortens, and below 480px the spine lays out inline and the credentials
 * strip drops, which is what makes it fit one screen on a phone.
 */
export function HomeV3Hero({ credentials }: { credentials: string[] }) {
  const hero = HOME_V3_HERO;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/*
          The five domains as nodes on one hairline rather than five detached
          pills. Below `nav` the line goes and the nodes wrap, since a single
          row no longer fits.
        */}
        <div
          data-hero-spine
